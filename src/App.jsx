      ...
      <div style={{ aspectRatio: '16/9', background: 'linear-gradient(135deg, #0a0a1a, #1a0a2e)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        {status === 'done' ? (
          ...
          <div style={{ textAlign: 'center', padding: '0 16px', width: '100%', boxSizing: 'border-box' }}>
            ...
            <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '6px' }}>~60–90 sec · Veo 3.1</div>
          </div>
        ) : status === 'failed' ? (
          <div style={{ textAlign: 'center' }}>
...
function VideoPlayer({ videoId, url, label, onClose }) {
  ...
  useEffect(() => {
    ...
    // For proxy URLs, fetch the video as a blob and create an object URL
    // This avoids all CORS and relative URL issues with <video src>
    const fullUrl = url.startsWith('/') ? `${window.location.origin}${url}` : url

    fetch(fullUrl)
      .then(r => {
        if (!r.ok) throw new Error(`Failed to load video: ${r.status}`)
        return r.blob()
      })
      .then(blob => {
      ...
      .catch(e => {
        console.error('[VideoPlayer] fetch error:', e.message)
        setError(e.message)
        setLoading(false)
      })
  ...
  useEffect(() => {
    if (blobUrl && videoRef.current) {
      ...
      videoRef.current.play().catch(e => console.warn('[VideoPlayer] autoplay:', e.message))
    }
  }, [blobUrl])

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: '#000', display: 'flex', flexDirection: 'column', animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', background: 'rgba(0,0,0,0.8)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div>
          <div style={{ fontSize: '10px', color: 'var(--gold)', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '2px' }}>Veo 3.1 · StarAlive</div>
          <div style={{ fontSize: '16px', fontWeight: 700 }}>{label}</div>
        </div>
        <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', borderRadius: '50%', width: '36px', height: '36px', fontSize: '18px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
      </div>

      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000', position: 'relative' }}>
        {loading && (
          <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}>
            <div style={{ width: '36px', height: '36px', border: '2px solid rgba(212,175,55,0.3)', borderTopColor: 'var(--gold)', borderRadius: '50%', animation: 'spin 0.9s linear infinite', margin: '0 auto 12px' }} />
            <div style={{ fontSize: '12px', letterSpacing: '0.1em' }}>Loading video...</div>
          </div>
        )}
        {error && (
          <div style={{ textAlign: 'center', color: 'rgba(255,80,80,0.8)', padding: '20px' }}>
            <div style={{ fontSize: '24px', marginBottom: '8px' }}>⚠️</div>
            <div style={{ fontSize: '13px' }}>Could not load video</div>
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginTop: '6px' }}>{error}</div>
          </div>
        )}
        {!loading && !error && (
          <video
      ...
      <div style={{ padding: '12px 20px', textAlign: 'center', fontSize: '11px', color: 'var(--text-muted)', background: 'rgba(0,0,0,0.8)' }}>
...
function VisionScene({ artistName, genre, onAdvance }) {
  ...
  function handleFile(file) {
    ...
    // Compress photo before storing — phone photos are 3-12MB which exceeds
    // Vercel's 4.5MB request body limit when base64 encoded.
    // Resize to 512x512 max at 85% JPEG quality = ~30-80KB — perfect for face reference
    const img = new Image()
    img.onload = () => {
      const MAX = 512
      ...
      // Strip prefix — store raw base64 only
      const base64 = dataUrl.split(',')[1]
      setPhotoBase64(base64)
      console.log(`Photo compressed: ${canvas.width}x${canvas.height}, ~${Math.round(base64.length * 0.75 / 1024)}KB`)
    }
